            ...
            <div className={`flex flex-col items-${isMe ? 'end' : 'start'}`}>
              ...
              {msg.reactions && Object.keys(msg.reactions).length > 0 && (
                <div className="flex flex-wrap gap-1 mt-1">
                  {EMOJIS.filter(emoji =>
                    Object.values(msg.reactions).some(userReacts => userReacts.includes(emoji))
                  ).map((emoji) => {
                    const count = Object.values(msg.reactions).filter(userReacts => userReacts.includes(emoji)).length;
                    const userReacted = msg.reactions[USER_ID]?.includes(emoji);
                    return (
                      <button
                        key={emoji}
                        onClick={() => addReaction(msg.message_id, emoji)}
